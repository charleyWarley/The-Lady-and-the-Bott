extends VBoxContainer

signal musicButton_pressed
signal crtButton_pressed
signal quitButton_pressed

onready var music = $musicButton
onready var crt = $crtButton
onready var quit = $quitButton

func _input(_event):
	if Input.is_action_just_pressed("pause"):
		music.set_pressed(true)
		crt.set_pressed(false)
		quit.set_pressed(false)
		return
	if !get_tree().is_paused(): return
	if Input.is_action_just_pressed("ui_accept"):
		if music.is_pressed(): emit_signal("musicButton_pressed")
		elif crt.is_pressed(): emit_signal("crtButton_pressed")
		elif quit.is_pressed(): emit_signal("quitButton_pressed")
		var buttons = get_children()
		for button in buttons: button.set_pressed(false)
		return
			
	if Input.is_action_just_pressed("ui_up") or Input.is_action_just_pressed("ui_down"):
		if music.is_pressed(): set_activeButton(music, crt)
		elif crt.is_pressed(): set_activeButton(crt, quit)
		elif quit.is_pressed(): set_activeButton(quit, music)

func set_activeButton(old, new):
	old.set_pressed(false)
	new.set_pressed(true)
